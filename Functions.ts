36.
function maake_shirt(size: number) {
...
40.
function make_album(artist_name: string, artist_song_title: string, tracks?: number) {
    if (tracks) {
        let music_Album1 = {
...
function Show_Magicians(fn: () => void, fn2: () => void) {
    fn();
    fn2();
}
...
44.
function Make_Sandwich(ingredients: string[], extras: string[]) {
    extras.join(" and ");
...
45.
function Car(model: string, manu: string, color?: string, alloywheels?: boolean) {
    let Make_Car = {
        model: model,
        manufacturur: manu,
        arbitrary_nums: {
